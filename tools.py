...
def search_flights(origin: str, destination: str) -> str:
    ...
    if not routes:
        # Thử chiều ngược — nhưng báo rõ là dữ liệu chiều ngược
        reverse = FLIGHTS_DB.get((destination, origin))
        if reverse:
            return (
                f"Không có dữ liệu trực tiếp cho tuyến {origin} → {destination}.\n"
                f"Dữ liệu tham khảo chiều ngược ({destination} → {origin}):\n"
                + "".join(
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    """Tìm kiếm khách sạn theo thành phố và ngân sách mỗi đêm."""
    ...
    for h in filtered:
        result += (
            ...
            f"{format_price(h['price_per_night'])}/đêm | Rating: {h['rating']}\n"
        )
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        return "Lỗi: format expenses không hợp lệ. Ví dụ đúng: 've_may_bay:890000,khach_san:650000'"
    ...
    for name, value in expense_dict.items():
        result += f"- {name.replace('_', ' ').title()}: {format_price(value)}\n"
